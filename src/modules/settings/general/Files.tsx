            ...
            <div className="w-full">
              ...
              >
                {({ getRootProps, getInputProps, open, isDragActive }) => (
                  <div
                    className={`border-default text-default dark:border-negative dark:text-negative relative flex w-full cursor-pointer flex-col items-center justify-center gap-2 rounded-t-md border-2 border-b-0 border-dashed px-4 py-8 ${isDragActive ? 'bg-offset dark:bg-negativeOffset' : 'bg-transparent'}`}
                    {...getRootProps()}
                  >
                    <input
                      {...getInputProps({
                    ...
                    {previewImage ? (
                      <Image
                        src={previewImage}
                        alt={file.name}
                        className="border-default dark:border-negative h-36 w-36 rounded-md border object-cover"
                        width={144}
                        height={144}
                      />
                    ) : (
                    ...
                    <button
                      type="button"
                      className="hover:text-offset dark:hover:text-negativeOffset text-sm"
                      onClick={open}
                    >
