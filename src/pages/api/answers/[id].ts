...
export default async function handler(
  ...
  if (req.method === 'PUT') {
    try {
      ...
      if (token) {
        const result = updateAnswerServerSchema.safeParse({body:req.body, query: req.query});
        if (result.success === false) {
          ...
          return res.status(201).json({success: true, message: 'Answer updated successfully' });
        }
      } else {
        return res
          .status(401)
