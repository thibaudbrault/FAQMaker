...
export default async function handler(
  ...
  if (req.method === 'GET') {
    try {
      ...
      if (token) {
        const result = getNodeServerSchema.safeParse({body:req.body, query: req.query});
    ...
    try {
      ...
      if (token) {
        const result = updateNodeServerSchema.safeParse({body:req.body, query: req.query});
        if (result.success === false) {
          const errors = result.error.formErrors.fieldErrors;
          return res.status(422).json({
            success: false,
          ...
          const { tenantId, questionId, text, slug, userId, tags } = result.data.body;
